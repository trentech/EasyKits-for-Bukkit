...
public abstract class SQLKits extends SQLUtils {
        ...
        try {
            ...
            while (result.next()) {
                if (result.getString("Kit").equalsIgnoreCase(name)) {
                    kit = Optional.of(new Kit(result.getString("Kit"), deserialize(result.getBytes("Inventory")), deserialize(result.getBytes("Armor")), result.getLong("Cooldown"),
                        result.getDouble("Price"), result.getInt("Limits")));
                }
            }
    ...
    public static ConcurrentHashMap<String, Kit> all() {
        ...
        try {
            ...
            while (result.next()) {
                Kit kit = new Kit(result.getString("Kit"), deserialize(result.getBytes("Inventory")), deserialize(result.getBytes("Armor")), result.getLong("Cooldown"), result.getDouble("Price"),
                    result.getInt("Limits"));
