...
public class CMDList {
    ...
    public static void execute(CommandSender sender) {
        ...
        for (Map.Entry<String, Kit> entry : list.entrySet()) {
            Kit kit = entry.getValue();

            if ((sender.hasPermission("easykits.kits." + kit.getName()) && !kit.getName().equalsIgnoreCase(Main.getPlugin().getConfig().getString("config.new-player-kit"))) || sender instanceof org.bukkit.command.ConsoleCommandSender) {
                ...
                if (sender instanceof Player) {
                    ...
                    if (price > 0.0D && !player.hasPermission("easyKits.override.price")) {
                        kitMsg = kitMsg + ":";
                    ...
                    if (limit > 0 && !player.hasPermission("easyKits.override.limit")) {
