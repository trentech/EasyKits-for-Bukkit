...
public class KitService {
    ...
    private void updateUsage(Kit kit, Player player) {
        ...
        if (!player.hasPermission("easykits.override.price") && kit.getPrice() > 0.0 && Main.getPlugin().getEconomy() != null) {
            Main.getPlugin().getEconomy().withdrawPlayer((OfflinePlayer) player, kit.getPrice());
        }
            ...
            for (ItemStack item : inv) {
                ...
                if (tempInv.getItem(index) == null) {
                    ...
                    if (size < item.getMaxStackSize()) {
                        ...
                        for (int i = 10; i <= 36; i++) {
                            if (i == 36) {
                                Notifications notifications = new Notifications("inventory-space", kit.getName(), player.getName());
                                player.sendMessage(notifications.getMessage());
                                return false;
                            }
                            size -= item.getMaxStackSize();
                            if (size < item.getMaxStackSize() && amount <= size) {
                                tempInv.setItem(index, item);
                            }
                        }
